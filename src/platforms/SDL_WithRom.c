...
LOCALFUNC blnr Sony_Insert1(char *drivepath, blnr silentfail)
{
	...
	char cwd[255]; // Buffer to hold the current working directory
...
LOCALFUNC blnr CreateMainWindow(void)
{
    ...
    gWindow = SDL_CreateWindow("Mini vMac", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, NewWindowWidth, NewWindowHeight, SDL_WINDOW_SHOWN | SDL_WINDOW_FULLSCREEN);
    gRenderer = SDL_CreateRenderer(gWindow, -1, SDL_RENDERER_SOFTWARE);
    my_surface = SDL_GetWindowSurface(gWindow);

    printf("window %p %p %p %d %d\n", gWindow, gRenderer, my_surface, NewWindowWidth, NewWindowHeight);
