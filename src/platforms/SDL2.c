...
{
    char *path = SDL_GetPrefPath("ru.erhoof", "minivmac");
    char boot_path[255];
    memset(boot_path, 0, sizeof(boot_path));
    strncat(boot_path, path, sizeof(boot_path) - strnlen(boot_path, sizeof(boot_path)) - 1);
    strncat(boot_path, "boot.img", sizeof(boot_path) - strnlen(boot_path, sizeof(boot_path)) - 1);

    Sony_Insert1(boot_path, falseblnr);
    if (! AnyDiskInserted()) {
        int i;
...
LOCALFUNC blnr LoadMacRom(void)
{
    ...
    memset(boot_path, 0, sizeof(boot_path));
    strncat(boot_path, path, sizeof(boot_path) - strnlen(boot_path, sizeof(boot_path)) - 1);
    strncat(boot_path, RomFileName, sizeof(boot_path) - strnlen(boot_path, sizeof(boot_path)) - 1);
...
#endif
    if (mnvm_fnfErr == (err = LoadMacRomFrom(boot_path)))
    {
    }
...
LOCALFUNC blnr CreateMainWindow(void)
{
    ...
    Uint32 flags = SDL_WINDOW_FULLSCREEN_DESKTOP /* SDL_WINDOW_HIDDEN */;
    blnr v = falseblnr;
        ...
        flags)))
    {
        fprintf(stderr, "SDL_CreateWindow fails: %s\n",
            SDL_GetError());
    } else if (NULL == (my_renderer = SDL_CreateRenderer(
        my_main_wind, -1,
        ...
        )))
    {
        fprintf(stderr, "SDL_CreateRenderer fails: %s\n",
            SDL_GetError());
    } else if (NULL == (my_texture = SDL_CreateTexture(
        my_renderer,
        ...
        )))
    {
        fprintf(stderr, "SDL_CreateTexture fails: %s\n",
            SDL_GetError());
    } else if (NULL == (my_format = SDL_AllocFormat(SDL_PIXELFORMAT_ARGB8888)))
    {
